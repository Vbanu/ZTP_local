...
$VIServer          = $data.vCenter.IP              # vCenter Server we're using
$VIUsername        = $data.vCenter.username  # user for vCenter
$VIPassword        = $data.vCenter.password                     # password for vCenter user

# Full Path to the vSAN Witness Appliance & Cluster
$vSANWitnessOVA    = $data.WitnessOVA.vSANWitnessOVA
$targetcluster     = $data.vCenter.clusterName 	                    # Cluster the OVA will be deployed to
$ntp1              = $data.WitnessOVA.ntp1          # NTP Host 1
$ntp2              = $data.WitnessOVA.ntp2          # NTP Host 2
$datastore 	   = $data.WitnessOVA.datastore               # Name of the Target Datastore

# Management Network Properties
$vmname            = $data.WitnessOVA.vmname         # Name of the Witness VM
$passwd            = $data.WitnessOVA.password                     # Witness VM root password
$dns1              = $data.WitnessOVA.dns1                  # DNS Server1
$dns2              = $data.WitnessOVA.dns2                  # DNS Server2
$hostname          = $data.WitnessOVA.hostname # DNS Address of the Witness VM
$dnsdomain         = $data.WitnessOVA.dnsdomain          # DNS Search Domain
$ipaddress0        = $data.WitnessOVA.ipaddress0                # IP address of VMK0, the Management VMK
$netmask0          = $data.WitnessOVA.netmask0               # Netmask of VMK0
$gateway0          = $data.WitnessOVA.gateway0                # Default System Gateway
$network0          = $data.WitnessOVA.network0                        # The network name that the Managment VMK will reside on

# Witness Network Properties
$ipaddress1        = $data.WitnessOVA.ipaddress1                  # IP address of VMK1, the WitnessPg VMK
$netmask1          = $data.WitnessOVA.netmask1                # Netmask of VMK1
$network1          = $data.WitnessOVA.network1                        # The network name that the Witness VMK will reside on
$deploymentsize    = $data.WitnessOVA.deploymentsize                         # The OVA deployment size. Options are "tiny","normal", and "large"
$witnessdatacenter = $data.vCenter.DCName                      # The Datacenter that the Witness Host will be added to 

# Witness Static Routes
$route1ip          = $data.vCenter.route1ip                   # IP/Range of Site A vSAN IP addresses
$route1gw          = $data.vCenter.route1gw                   # Gateway to Site A that VMK1 will use
$route1pfx         = $data.vCenter.route1pfx                           # Network Prefix

$route2ip          = $data.vCenter.route2ip                   # IP/Range of Site B vSAN IP addresses
$route2gw          = $data.vCenter.route2gw                   # Gateway to Site B that VMK1 will use
$route2pfx         = $data.vCenter.route2pfx                           # Network Prefix

# vSAN Traffic Tagged NIC Selection 
$vsannetwork       = $data.vCenter.vsannetwork                    # 'Management' or 'Secondary' NIC
